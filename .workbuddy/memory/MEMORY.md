...
- 网格策略的"总资产"= wallet-group 中所有分组的 totalBalance 之和（主要来自 TradingBots + Futures）
...
- **推送标题已加入总资产**：getTotalAssets() 函数在 index.js 中，推送时调用并传入 total_assets 到 Bark 推送标题（🏦{total_assets}U，保留整数）
